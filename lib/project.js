...
function NewProject (options) {
  ...
  p.starters = options.starters.map(function (options) {
    if (!options.module) {
      options = { module: options };
    }

    var s = options.module(p);
    s.subfolder = options.subfolder;
...
function create (project, callback) {
  ...
  serial()
    ...
    .then(project.createSubfolders)
    .then(project.start)
    .done(callback);
}

function createSubfolders (project, callback) {
  loop(project.starters.length, each, callback);

  function each (done, index) {
    if (!project.starters[index].subfolder) return done();

    var target = project.starters[index].targetFolder();
