...
var CACHE_DIR = expandHomeDir('~/.hek/cache');
...
function getModule (name) {
  debug('Getting module "%s"', name, normalizeModuleName(name));
  return require(path.join(CACHE_DIR, 'node_modules', normalizeModuleName(name)));
}
...
function addToIndex (listName, names, callback) {
  ...
  readIndex(function (error, index) {
    ...
    names.forEach(function (n) {
      delete index['installed'][n];
      delete index['linked'][n];
