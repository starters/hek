...
var command = require("new-command")('start', 'install', 'link', 'unlink', 'list', {});
...
function run () {
  ...
  console.error('\n  Unknown command: %s. List all options by kik -h command.\n', command._subcommand || command._[1]);
}
...
function list () {
  cache.list(function (error, result) {
    ...
    console.log(format('\n  {bold}%d{reset} installed, {bold}%d{reset} linked modules.'), result.installed.length, result.linked.length);
